...
AI_MODEL_NAME = "gpt-4o-mini"  # 필요시 gpt-4o 등 다른 모델로 교체 가능


def ask_ai(prompt):
    """ChatGPT(OpenAI) 호출 공통 함수. 성공 시 (True, 텍스트), 실패 시 (False, 에러메시지) 반환."""
    if not ai_ready:
    ...
    try:
        ...
        if "quota" in err_msg or "429" in err_msg or "rate limit" in err_msg:
            return False, "⚠️ OpenAI API 호출 제한(쿼터/레이트리밋)에 도달했습니다. 잠시 후 다시 시도해주세요."
...
with tab1:
    ...
    with st.spinner("ChatGPT가 실시간 경제 뉴스를 분석 중입니다..."):
