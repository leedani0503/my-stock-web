...
# 📡 0. 한국거래소(KRX) 전체 종목코드 불러오기 (자동완성용)
...
def load_krx_stock_list():
    try:
        ...
        # 딕셔너리로 변환: {'삼성전자': '005930', '카카오': '035720', ...}
        return dict(zip(df['회사명'], df['종목코드']))
    except Exception as e:
        # 혹시 사이트 접속이 안될 경우를 대비한 기본 종목들
        return {"삼성전자": "005930", "SK하이닉스": "000660", "현대차": "005380", "NAVER": "035420", "카카오": "035720", "셀트리온": "068270", "POSCO홀딩스": "005490"}

# 📡 1. 뉴스 수집 및 시간/분 파싱 (최신순 정렬 추가)
def fetch_google_news(query, max_results=40):  # 기본 뉴스 수집량을 40개로 늘림
...
with tab1:
    ...
    with st.spinner("구글 AI가 실시간 경제 뉴스를 분석 중입니다..."):
        # 더 많은 최신 뉴스를 가져오기 위해 max_results=40 세팅
        ...
        if not economy_news_df.empty:
            if ai_ready:
                ...
                # 요약할 때는 상위 20개만 넣어서 구글 AI 글자수 제한(Quota) 초과 방지
...
with tab2:
    ...
    # 📌 종목명 자동완성 로직 (Selectbox를 활용한 검색 기능)
    with st.spinner("한국거래소(KRX) 상장 종목 목록을 불러오는 중..."):
        krx_dict = load_krx_stock_list()
    
    # Selectbox 자체가 자동완성 검색창 역할을 수행합니다!
